...
import { flatten, isString, cloneDeep, sortBy } from "lodash"
...
import { findByName, getIgnorer, scanDirectory, pickKeys, Ignorer } from "./util/util"
import { MODULE_CONFIG_FILENAME } from "./constants"
import { ConfigurationError, PluginError, RuntimeError } from "./exceptions"
import { VcsHandler, ModuleVersion, NEW_MODULE_VERSION } from "./vcs/base"
...
import { BuildDependencyConfig, ModuleConfig, baseModuleSpecSchema } from "./config/module"
...
export class Garden {
  ...
  private actionHandler: ActionHelper
  ...
  async resolveProviders(): Promise<Provider[]> {
    await asyncLock.acquire("resolve-providers", async () => {
      ...
      // TODO: Detect circular deps here

      const tasks = await Bluebird.map(rawConfigs, async (config) => {
  ...
  async getActionHandler() {
    if (!this.actionHandler) {
      ...
      this.actionHandler = new ActionHelper(this, providers)
    }
   ...
   */
  private async getRawModuleConfigs(keys?: string[]): Promise<ModuleConfig[]> {
  ...
  async resolveModuleConfigs(keys?: string[], opts: ModuleConfigResolveOpts = {}): Promise<ModuleConfig[]> {
    const actions = await this.getActionHandler()
